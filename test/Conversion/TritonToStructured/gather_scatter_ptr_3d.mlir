...
// CHECK:           %[[MAKE_GATHER_SCATTER_TPTR_0:.*]] = tts.make_gather_scatter_tptr %[[ARG0]] to sizes: [32, 32, 32] gather_scatter_dim: 1 gather_scatter_offset: %[[VAL_0]], strides: {{\[}}%[[INDEX_CAST_0]], %[[INDEX_CAST_2]], %[[INDEX_CAST_1]]], offsets: [0, 0, 0] : tensor<32xi32>  <f32> to tensor<32x32x32x!tt.ptr<f32>>
// CHECK:           %[[VAL_1:.*]] = "tts.load"(%[[MAKE_GATHER_SCATTER_TPTR_0]]) <{operandSegmentSizes = array<i32: 1, 0, 0>, static_mask_dims = array<i64>}> : (tensor<32x32x32x!tt.ptr<f32>>) -> tensor<32x32x32xf32>
// CHECK:           %[[MAKE_GATHER_SCATTER_TPTR_1:.*]] = tts.make_gather_scatter_tptr %[[ARG2]] to sizes: [32, 32, 32] gather_scatter_dim: 1 gather_scatter_offset: %[[VAL_0]], strides: {{\[}}%[[INDEX_CAST_0]], %[[INDEX_CAST_2]], %[[INDEX_CAST_1]]], offsets: [0, 0, 0] : tensor<32xi32>  <f32> to tensor<32x32x32x!tt.ptr<f32>>
// CHECK:           "tts.store"(%[[MAKE_GATHER_SCATTER_TPTR_1]], %[[VAL_1]]) <{static_mask_dims = array<i64>}> : (tensor<32x32x32x!tt.ptr<f32>>, tensor<32x32x32xf32>) -> ()
// CHECK:           tt.return
// CHECK:         }
module attributes {} {
