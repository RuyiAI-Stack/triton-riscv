...
// CHECK:           %[[MAKE_TPTR_0:.*]] = tts.make_tptr %[[ARG0]] to sizes: [4, 256], strides: [1, %[[CONSTANT_1]]], offsets: {{\[}}%[[INDEX_CAST_0]], 0], shape: [0, 0], order: [] : <bf16> to tensor<4x256x!tt.ptr<bf16>>
...
// CHECK:           %[[FOR_0:.*]]:2 = scf.for %[[VAL_1:.*]] = %[[CONSTANT_4]] to %[[CONSTANT_3]] step %[[CONSTANT_2]] iter_args(%[[VAL_2:.*]] = %[[VAL_0]], %[[VAL_3:.*]] = %[[INDEX_CAST_0]]) -> (tensor<4x256xbf16>, index) {
// CHECK:             %[[MAKE_TPTR_1:.*]] = tts.make_tptr %[[ARG1]] to sizes: [4, 256], strides: {{\[}}%[[CONSTANT_0]], %[[CONSTANT_1]]], offsets: {{\[}}%[[VAL_3]], %[[CONSTANT_4]]], shape: [0, 0], order: [] : <bf16> to tensor<4x256x!tt.ptr<bf16>>
// CHECK:             %[[VAL_4:.*]] = "tts.load"(%[[MAKE_TPTR_1]]) <{operandSegmentSizes = array<i32: 1, 0, 0>, static_mask_dims = array<i64>}> : (tensor<4x256x!tt.ptr<bf16>>) -> tensor<4x256xbf16>
// CHECK:             %[[ADDF_0:.*]] = arith.addf %[[VAL_2]], %[[VAL_4]] : tensor<4x256xbf16>
// CHECK:             %[[ADDI_0:.*]] = arith.addi %[[VAL_3]], %[[CONSTANT_2]] : index
// CHECK:             scf.yield %[[ADDF_0]], %[[ADDI_0]] : tensor<4x256xbf16>, index
// CHECK:           }
// CHECK:           %[[MAKE_TPTR_2:.*]] = tts.make_tptr %[[ARG2]] to sizes: [4, 256], strides: [1, %[[CONSTANT_1]]], offsets: {{\[}}%[[INDEX_CAST_0]], 0], shape: [0, 0], order: [] : <bf16> to tensor<4x256x!tt.ptr<bf16>>
// CHECK:           "tts.store"(%[[MAKE_TPTR_2]], %[[VAL_5:.*]]#0) <{static_mask_dims = array<i64>}> : (tensor<4x256x!tt.ptr<bf16>>, tensor<4x256xbf16>) -> ()
// CHECK:           tt.return
// CHECK:         }
  tt.func @kernel(
    %arg0 : !tt.ptr<bf16>,
