...
// CHECK:           %[[CONSTANT_4:.*]] = arith.constant 0 : i8
// CHECK:           %[[CONSTANT_5:.*]] = arith.constant 0 : index
// CHECK:           %[[CONSTANT_6:.*]] = arith.constant 8 : index
// CHECK:           %[[CONSTANT_7:.*]] = arith.constant dense<0> : tensor<16xi32>
// CHECK:           %[[CONSTANT_8:.*]] = arith.constant dense<0> : tensor<8xi32>
...
// CHECK:           %[[MINSI_0:.*]] = arith.minsi %[[INDEX_CAST_0]], %[[CONSTANT_6]] : index
// CHECK:           %[[MAXSI_0:.*]] = arith.maxsi %[[MINSI_0]], %[[CONSTANT_5]] : index
// CHECK:           %[[VAL_0:.*]] = "tts.load"(%[[MAKE_TPTR_0]], %[[MAXSI_0]], %[[CONSTANT_4]]) <{operandSegmentSizes = array<i32: 1, 1, 1>, static_mask_dims = array<i64: -9223372036854775808>}> : (tensor<8x!tt.ptr<i8>>, index, i8) -> tensor<8xi8>
// CHECK:           %[[EXTSI_0:.*]] = arith.extsi %[[VAL_0]] : tensor<8xi8> to tensor<8xi32>
// CHECK:           %[[CMPI_0:.*]] = arith.cmpi ne, %[[EXTSI_0]], %[[CONSTANT_8]] : tensor<8xi32>
...
// CHECK:           %[[MAXSI_1:.*]] = arith.maxsi %[[MINSI_1]], %[[CONSTANT_5]] : index
// CHECK:           %[[VAL_1:.*]] = "tts.load"(%[[MAKE_TPTR_1]], %[[MAXSI_1]], %[[CONSTANT_4]]) <{operandSegmentSizes = array<i32: 1, 1, 1>, static_mask_dims = array<i64: -9223372036854775808>}> : (tensor<16x!tt.ptr<i8>>, index, i8) -> tensor<16xi8>
// CHECK:           %[[EXTSI_1:.*]] = arith.extsi %[[VAL_1]] : tensor<16xi8> to tensor<16xi32>
// CHECK:           %[[CMPI_1:.*]] = arith.cmpi ne, %[[EXTSI_1]], %[[CONSTANT_7]] : tensor<16xi32>
// CHECK:           %[[INDEX_CAST_2:.*]] = arith.index_cast %[[ARG7]] : i32 to index
// CHECK:           %[[INDEX_CAST_3:.*]] = arith.index_cast %[[ARG8]] : i32 to index
// CHECK:           %[[INDEX_CAST_4:.*]] = arith.index_cast %[[ARG4]] : i32 to index
// CHECK:           %[[MINSI_2:.*]] = arith.minsi %[[INDEX_CAST_4]], %[[CONSTANT_2]] : index
// CHECK:           %[[MAXSI_2:.*]] = arith.maxsi %[[MINSI_2]], %[[CONSTANT_5]] : index
...
// CHECK:           %[[MAKE_GATHER_SCATTER_TPTR_1:.*]] = tts.make_gather_scatter_tptr %[[ARG1]] to sizes: [4, 8, 16] gather_scatter_dim: 2 gather_scatter_offset: %[[MAKE_RANGE_1]] gather_scatter_mask: %[[CMPI_1]], strides: {{\[}}%[[INDEX_CAST_2]], %[[INDEX_CAST_3]], 1], offsets: [0, 0, 0] : tensor<16xi32> tensor<16xi1> <f32> to tensor<4x8x16x!tt.ptr<f32>>
// CHECK:           "tts.store"(%[[MAKE_GATHER_SCATTER_TPTR_1]], %[[VAL_2]], %[[MINSI_4]], %[[MINSI_7]]) <{static_mask_dims = array<i64: -9223372036854775808, -9223372036854775808, 0>}> : (tensor<4x8x16x!tt.ptr<f32>>, tensor<4x8x16xf32>, index, index) -> ()
// CHECK:           tt.return
// CHECK:         }
module {
