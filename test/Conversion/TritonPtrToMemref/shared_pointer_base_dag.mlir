...
module {
  func.func @shared_pointer_base_dag(
    ...
    %value = tt.load %s39 : !tt.ptr<f32>
    tt.store %out, %value : !tt.ptr<f32>
    return
  }
}

// CHECK-LABEL: func.func @shared_pointer_base_dag(
// CHECK-COUNT-39: arith.select
// CHECK: %[[BASE:.*]] = arith.select %{{.*}}, %{{.*}}, %{{.*}} : memref<*xf32>
// CHECK: builtin.unrealized_conversion_cast %[[BASE]] : memref<*xf32> to !tt.ptr<f32>
