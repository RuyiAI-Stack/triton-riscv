...
// CHECK:           %[[REINTERPRET_CAST_0:.*]] = memref.reinterpret_cast %[[ARG0]] to offset: [0], sizes: [4, 4], strides: {{\[}}%[[INDEX_CAST_1]], %[[INDEX_CAST_2]]] : memref<*xf32> to memref<4x4xf32, strided<[?, ?]>>
// CHECK:           %[[ALLOC_0:.*]] = memref.alloc() : memref<4x4xf32>
// CHECK:           memref.copy %[[REINTERPRET_CAST_0]], %[[ALLOC_0]] : memref<4x4xf32, strided<[?, ?]>> to memref<4x4xf32>
// CHECK:           %[[TO_TENSOR_0:.*]] = bufferization.to_tensor %[[ALLOC_0]] restrict writable : memref<4x4xf32> to tensor<4x4xf32>
// CHECK:           %[[EMPTY_1:.*]] = tensor.empty() : tensor<4x4xi32>
// CHECK:           %[[GENERIC_1:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_1]], #[[$ATTR_2]]], iterator_types = ["parallel", "parallel"]} ins(%[[EXPAND_SHAPE_0]] : tensor<1x4xi32>) outs(%[[EMPTY_1]] : tensor<4x4xi32>) attrs =  {broadcastDims = array<i64: 0>} {
...
// CHECK:               %[[CMPF_1:.*]] = arith.cmpf ogt, %[[VAL_3]], %[[VAL_5]] : f32
// CHECK:               %[[ORI_0:.*]] = arith.ori %[[CMPF_1]], %[[ANDI_0]] : i1
// CHECK:               %[[SELECT_0:.*]] = arith.select %[[ORI_0]], %[[VAL_3]], %[[VAL_5]] : f32
...
// CHECK:           %[[TO_TENSOR_0:.*]] = bufferization.to_tensor %[[ALLOC_0]] restrict writable : memref<4x4xf32> to tensor<4x4xf32>
// CHECK:           %[[EMPTY_1:.*]] = tensor.empty() : tensor<4x4xi32>
// CHECK:           %[[GENERIC_1:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_4]], #[[$ATTR_5]]], iterator_types = ["parallel", "parallel"]} ins(%[[EXPAND_SHAPE_0]] : tensor<1x4xi32>) outs(%[[EMPTY_1]] : tensor<4x4xi32>) attrs =  {broadcastDims = array<i64: 0>} {
// CHECK:           ^bb0(%[[VAL_1:.*]]: i32, %[[VAL_2:.*]]: i32):
// CHECK:             linalg.yield %[[VAL_1]] : i32
// CHECK:           } -> tensor<4x4xi32>
// CHECK:           %[[EMPTY_2:.*]] = tensor.empty() : tensor<4xf32>
...
// CHECK:               %[[CMPI_0:.*]] = arith.cmpi slt, %[[VAL_4]], %[[VAL_6]] : i32
// CHECK:               %[[ANDI_0:.*]] = arith.andi %[[CMPF_0]], %[[CMPI_0]] : i1
// CHECK:               %[[CMPF_1:.*]] = arith.cmpf olt, %[[VAL_3]], %[[VAL_5]] : f32
// CHECK:               %[[ORI_0:.*]] = arith.ori %[[CMPF_1]], %[[ANDI_0]] : i1
// CHECK:               %[[SELECT_0:.*]] = arith.select %[[ORI_0]], %[[VAL_3]], %[[VAL_5]] : f32
// CHECK:               %[[SELECT_1:.*]] = arith.select %[[ORI_0]], %[[VAL_4]], %[[VAL_6]] : i32
// CHECK:               linalg.yield %[[SELECT_0]], %[[SELECT_1]] : f32, i32
