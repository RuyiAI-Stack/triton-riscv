...
>
> **Best for:** compiler contributors improving generated code after
> correctness is established.
>
> **Prerequisites:** [Implementation details](02-Implementation.md),
> [Inspecting IR](03-IR.md), and a passing focused correctness test.
>
> **After this guide:** you can identify expensive IR patterns, choose a safe
> optimization target, and define correctness, IR-shape, and performance checks.

This document is a roadmap for improving generated code after correctness is
established. It records bottlenecks found while inspecting RISC-V lowering,
updating MLIR tests, and validating standalone RVV executables.

A passing test proves only the behavior asserted by that test. It does not
prove that the compiler avoided allocations, copies, scalar fallbacks, or weak
memory-access patterns. Every optimization in this document must preserve
pointer, mask, bounds, stride, aliasing, and control-flow semantics.

## How to Use This Roadmap

| If you observe… | Start with |
| --- | --- |
| `memref.alloc` + `memref.copy` before computation | [Load-side temporary buffers](#2-eliminate-load-side-temporary-buffers) |
| a computed temporary copied into the real output | [Destination passing](#3-destination-passing-and-buffer-reuse) |
| scalar code for a regular elementwise kernel | [Direct RVV loads and stores](#4-prefer-direct-rvv-loads-and-stores) |
| incorrect or illegal gather/scatter conversion | [Pointer representation](#5-gatherscatter-pointer-representation) |
| pointer state lost across `scf.if` or loops | [Conditional pointer tensors](#6-conditional-pointer-tensor-lowering) |
| removed coverage after Triton API changes | [Tensor descriptors](#7-tensor-descriptor-test-coverage) |
| brittle tests that fail on harmless IR churn | [FileCheck quality](#8-filecheck-quality) |

Use [Inspecting IR](03-IR.md) to find the first stage containing the unwanted
form, and use [Benchmarking](08-Benchmark.md) only after correctness and code
shape checks pass.
