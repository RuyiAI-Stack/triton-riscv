...
struct MatmulConverter : public OpConversionPattern<triton::DotOp> {
  ...
  matchAndRewrite(triton::DotOp op, OpAdaptor adaptor,
    ...
    auto bElemType = cast<RankedTensorType>(opb.getType()).getElementType();
...
private:
  ...
  bool requiresF32Conversion(const Type elemType, Operation *redOp) const {
    if (auto floatType = dyn_cast<FloatType>(elemType)) {
      ...
      bool opOk = isa<arith::AddFOp>(redOp) || isa<arith::MulFOp>(redOp) ||
                  isa<arith::MaximumFOp>(redOp) || isa<arith::MinimumFOp>(redOp);
      return widthOk && opOk;
    }
    return false;
  }
  ...
  Value getRedElement(Value lhs, Value rhs, const Location loc,
    ...
    if (auto addf = dyn_cast<arith::AddFOp>(redOp)) {
      ...
      return b.create<arith::AddFOp>(loc, accType, lhs, rhs,
                                     addf.getFastmathAttr());
    }
    if (auto mulf = dyn_cast<arith::MulFOp>(redOp)) {
      if (lhs.getType() != accType)
        lhs = b.create<arith::ExtFOp>(loc, accType, lhs);
      return b.create<arith::MulFOp>(loc, accType, lhs, rhs,
                                     mulf.getFastmathAttr());
    }
    if (auto maxf = dyn_cast<arith::MaximumFOp>(redOp)) {
      if (lhs.getType() != accType)
        lhs = b.create<arith::ExtFOp>(loc, accType, lhs);
      return b.create<arith::MaximumFOp>(loc, accType, lhs, rhs,
                                         maxf.getFastmathAttr());
    }
    if (auto minf = dyn_cast<arith::MinimumFOp>(redOp)) {
      if (lhs.getType() != accType)
        lhs = b.create<arith::ExtFOp>(loc, accType, lhs);
      return b.create<arith::MinimumFOp>(loc, accType, lhs, rhs,
                                         minf.getFastmathAttr());
    }

    return llvm::TypeSwitch<Operation *, Value>(redOp)
        .Case<arith::AddIOp, arith::MulIOp, arith::AndIOp, arith::OrIOp,
              arith::XOrIOp, arith::MaxSIOp, arith::MaxUIOp, arith::MinSIOp,
              arith::MinUIOp, arith::MinimumFOp, arith::MinNumFOp,
  ...
  SmallVector<Value> cloneReduceBody(triton::ReduceOp op, ValueRange current,
    ...
    for (auto [arg, value] : llvm::zip_equal(
             reduceBlock->getArguments().take_front(current.size()), current)) {
      mapping.map(arg, value);
    }
    for (auto [arg, value] :
         llvm::zip_equal(reduceBlock->getArguments().drop_front(current.size()),
                         accumulated)) {
      mapping.map(arg, value);
    }
  ...
  convertToLinalgReduce(triton::ReduceOp op,
    ...
    bool isSimpleReduction = (reductionOps.size() == 1 &&
                              operands.size() == 1 &&
    ...
    auto linalgOp = rewriter.create<linalg::ReduceOp>(
        ...
        [&](OpBuilder &b, Location loc, ValueRange inputs) {
          ...
          if (isSimpleReduction) {
            auto rop = reductionOps.front();
            Value result = getRedElement(current[0], accumulated[0], loc, rop, b,
...
public:
  ...
  matchAndRewrite(triton::ScanOp op, OpAdaptor adaptor,
    ...
    if (rank == 1) {
      ...
      for (Value input : inputs)
        initialAccs.push_back(
            rewriter.create<tensor::ExtractOp>(loc, input, ValueRange{c0}));
      ...
      for (auto [initTensor, initialAcc] :
        ...
        firstResults.push_back(rewriter.create<tensor::InsertOp>(
            loc, initialAcc, initTensor, ValueRange{c0}));
      }
      ...
      auto loop = rewriter.create<scf::ForOp>(loc, c1, n, c1, iterArgs);
      {
        ...
        for (Value input : inputs)
          current.push_back(
              rewriter.create<tensor::ExtractOp>(loc, input, ValueRange{i}));
        ...
        for (auto [resultTensor, nextAcc] : llvm::zip_equal(
          ...
          nextResults.push_back(rewriter.create<tensor::InsertOp>(
              loc, nextAcc, resultTensor, ValueRange{i}));
        }
    ...
    auto outerLoop = rewriter.create<scf::ForOp>(loc, c0, m, c1, initTensors);
    {
      ...
      for (Value input : inputs)
        initialAccs.push_back(
            rewriter.create<tensor::ExtractOp>(loc, input, ValueRange{i, c0}));
      ...
      for (auto [outputTensor, initialAcc] :
        ...
        firstResults.push_back(rewriter.create<tensor::InsertOp>(
            loc, initialAcc, outputTensor, ValueRange{i, c0}));
      }
      ...
      auto innerLoop = rewriter.create<scf::ForOp>(loc, c1, n, c1, iterArgs);
      {
        ...
        for (Value input : inputs)
          current.push_back(
              rewriter.create<tensor::ExtractOp>(loc, input, ValueRange{i, j}));
        ...
        for (auto [resultTensor, nextAcc] : llvm::zip_equal(
          ...
          nextResults.push_back(rewriter.create<tensor::InsertOp>(
              loc, nextAcc, resultTensor, ValueRange{i, j}));
        }
