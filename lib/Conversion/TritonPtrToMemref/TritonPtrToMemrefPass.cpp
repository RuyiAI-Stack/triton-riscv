...
static bool
isScalarPointerBaseSelectionImpl(Value value,
                                 const llvm::DenseSet<Block *> &entryBlocks,
                                 llvm::SmallDenseSet<Value> &knownBases) {
  if (knownBases.contains(value)) {
    return true;
  }

  auto remember = [&](bool isBase) {
    if (isBase) {
      knownBases.insert(value);
    }
    return isBase;
  };
  ...
  if (auto cast = value.getDefiningOp<UnrealizedConversionCastOp>()) {
    if (cast.getInputs().size() == 1 && cast->getNumResults() == 1) {
      return remember(isScalarPointerBaseSelectionImpl(
          cast.getInputs().front(), entryBlocks, knownBases));
    }
  }
  ...
  if (auto select = value.getDefiningOp<arith::SelectOp>()) {
    return remember(isScalarPointerBaseSelectionImpl(select.getTrueValue(),
                                                     entryBlocks, knownBases) &&
                    isScalarPointerBaseSelectionImpl(select.getFalseValue(),
                                                     entryBlocks, knownBases));
  }
  ...
  auto isBaseYield = [&](Region &region) {
    ...
    return yield && resultIndex < yield.getNumOperands() &&
           isScalarPointerBaseSelectionImpl(yield.getOperand(resultIndex),
                                            entryBlocks, knownBases);
  };
  return remember(isBaseYield(ifOp.getThenRegion()) &&
...
isScalarPointerBaseSelection(Value value,
                             const llvm::DenseSet<Block *> &entryBlocks) {
  // Keep successful DAG visits local to this query: conversion rewrites can
  // change the selected values between legality checks.
  llvm::SmallDenseSet<Value> knownBases;
  return isScalarPointerBaseSelectionImpl(value, entryBlocks, knownBases);
}
