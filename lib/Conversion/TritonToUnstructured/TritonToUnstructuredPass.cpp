...
public:
  ...
  LogicalResult processUnstructuredPtrs(ModuleOp root,
    ...
    auto isScalarBaseSelectionImpl =
        [&](auto &&self, Value value,
            llvm::SmallDenseSet<Value> &knownBases) -> bool {
      ...
      if (auto select = value.getDefiningOp<arith::SelectOp>()) {
        return remember(self(self, select.getTrueValue(), knownBases) &&
                        self(self, select.getFalseValue(), knownBases));
      }
      ...
      auto isBaseYield = [&](Region &region) {
        ...
        return yield && resultIndex < yield.getNumOperands() &&
               self(self, yield.getOperand(resultIndex), knownBases);
      };
      return remember(isBaseYield(ifOp.getThenRegion()) &&
                      isBaseYield(ifOp.getElseRegion()));
    };

    auto isScalarBaseSelection = [&](Value value) {
      // The worklist rewrites pointer operands, so do not retain answers
      // between queries. Shared subexpressions are visited once per query.
      llvm::SmallDenseSet<Value> knownBases;
      return isScalarBaseSelectionImpl(isScalarBaseSelectionImpl, value,
                                       knownBases);
    };
        ...
        auto res =
            llvm::TypeSwitch<Operation *, LogicalResult>(user)
                ...
                .Case<arith::SelectOp>([&](arith::SelectOp select) {
                  ...
                  // selected input.
                  if (!isScalarBaseSelection(result)) {
                    return failure();
                  }
