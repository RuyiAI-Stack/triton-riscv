...
from .randint import wide_randint_kernel
...
def randint_like(
    ...
    *,
    ...
    if high <= 0:
        raise RuntimeError("random_ expects high to be greater than zero")
    ...
    # Float32 scaling cannot represent every integer above this range.
    unroll = 1 if high > 2**24 else UNROLL
    kernel = wide_randint_kernel if high > 2**24 else randint_kernel
    ...
    kernel[grid_fn](out, N, high, philox_seed, philox_offset, BLOCK=BLOCK)
    return out
