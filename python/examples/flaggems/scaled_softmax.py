...
def scaled_softmax_backward_kernel(
    ...
    for k_block_idx in range(0, tl.cdiv(key_seq_len, BLOCK_K)):
        ...
        P_block = tl.load(
            ptr_P, mask=mask, other=0.0, eviction_policy="evict_first"
        ).to(tl.float32)
        dP_block = tl.load(
            ptr_dP, mask=mask, other=0.0, eviction_policy="evict_first"
        ).to(tl.float32)
        ...
        tl.store(
            ptr_dS,
            dS_block.to(grad_input_ptr.dtype.element_ty),
            mask=mask,
            cache_modifier=".cs",
        )
...
def scaled_softmax(input_t: torch.Tensor, scale_factor: float):
    return scaled_softmax_forward(input_t, scale_factor)
