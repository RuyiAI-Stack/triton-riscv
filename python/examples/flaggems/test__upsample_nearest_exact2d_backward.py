...
@pytest.mark.parametrize("input_size", [(1, 1, 2, 3), (1, 1, 3, 4)])
def test_upsample_nearest_exact2d_backward_strided_gradient(input_size):
    grad = torch.arange(6, dtype=torch.float32).reshape(1, 1, 3, 2).transpose(-1, -2)
    assert not grad.is_contiguous()
    expected = torch.ops.aten._upsample_nearest_exact2d_backward(
        grad, [2, 3], input_size, None, None
    )
    actual = _upsample_nearest_exact2d_backward(grad, (2, 3), input_size)
