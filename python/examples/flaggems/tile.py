...
def generate_functional_tile_wrapper(
    ...
    code += "    dims_rank = len(dims)\n"
    code += "    in0_shape = list(in0.shape)\n"
    code += "    dims_shape = list(dims)\n\n"
    code += "    if dims_rank < in0_rank:\n"
    code += "        diff = in0_rank - dims_rank\n"
    code += "        ones = [1 for _ in range(diff)]\n"
    code += "        dims_shape = ones + dims_shape\n"
    code += "    elif dims_rank > in0_rank:\n"
    code += "        diff = dims_rank - in0_rank\n"
    code += "        ones = [1 for _ in range(diff)]\n"
    code += "        in0_shape = ones + in0_shape\n\n"
    code += "    is_empty = False\n"
    code += "    out_shape = []\n"
    code += "    for i in range(len(in0_shape)):\n"
    code += "        assert dims_shape[i] >= 0, 'the number of repetitions per dimension out of range (expected to >= 0) but got {}'.format(dims_shape[i])\n"
    code += "        if dims_shape[i] == 0:\n"
    code += "            is_empty = True\n"
    code += "        out_shape.append(in0_shape[i] * dims_shape[i])\n"
...
def generate_destination_passing_tile_wrapper(
    ...
    if rank > 0:
        ...
        code += (
            ...
            + ",  # task indexing space when input and output tensor has different shape\n"
        )
        code += "        num_tasks,\n"
