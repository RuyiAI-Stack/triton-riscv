...
FLASH_BWD_BLOCK_M = 64
FLASH_BWD_BLOCK_N = 64
...
def _flash_attn_bwd_dq_fused(
    ...
    for start_n in range(start_n_min, start_n_max):
        ...
        if IS_DROPOUT:
            # Reuse the forward Philox stream, byte threshold and lane ordering.
            drop_mask = (
                apply_dropout(
                    tl.full((BLOCK_M, BLOCK_N), 1, tl.float32),
                    start_m * BLOCK_M,
                    start_n * BLOCK_N,
                    seqlen_k,
                    batch_idx,
...
def _flash_attn_bwd_dkv(
    ...
    for linear in range(total_iters):
        ...
        p_drop = p
        if IS_DROPOUT:
            p_drop = tl.where(drop_mask, p * drop_scale, 0.0)
        dv = dv + tl.dot(tl.trans(p_drop.to(q.dtype)), do)
...
def flash_attn_backward(
    ...
    if use_dropout:
        ...
        p_dropout_uint8 = math.floor((1.0 - dropout_p) * 255.0)
    else:
    ...
    if do_bias_grad:
        # Causal/window masks can skip entire tiles, whose gradients are zero.
    ...
    if is_varlen:
        ...
        grid_dq = (triton.cdiv(SeqLen_q, FLASH_BWD_BLOCK_M), Batch, H_q)
        _flash_attn_bwd_varlen_dq_fused[grid_dq](
            ...
            sb_h,
            sb_n,
            BLOCK_M=FLASH_BWD_BLOCK_M,
            BLOCK_N=FLASH_BWD_BLOCK_N,
            HEAD_DIM=Head_Dim,
            IS_CAUSAL=is_causal,
        ...
        grid_dkv = (triton.cdiv(SeqLen_k, FLASH_BWD_BLOCK_N), Batch, H_k)
...
def scaled_dot_product_flash_attention_backward(
    ...
    *,
    ...
    # Packed variable-length inputs are already THD; dense ATen inputs are BHSD.
    if not use_varlen:
...
def scaled_dot_product_cudnn_attention_backward(
    ...
    *,
    ...
    dQ_bshd, dK_bshd, dV_bshd, _dBias = flash_attn_backward(
        grad_out_bshd,
        query_bshd,
        key_bshd,
        value_bshd,
