...
def _searchsorted_kernel(
    ...
    # Equivalent lower-bound walk that avoids the backend's mislowering of a
    # loop-carried midpoint pointer expression.
    if USE_INT32_INDEX:
        idx = tl.zeros((BLOCK_SIZE,), dtype=tl.int32)
    else:
        idx = tl.zeros((BLOCK_SIZE,), dtype=tl.int64)

    for i in range(LOG_SEQUENCE_LEN):
        step = 1 << (LOG_SEQUENCE_LEN - i - 1)
        next_idx = idx + step
        search_active = mask & (next_idx <= sequence_len)
        sorted_offsets = row_offsets + next_idx - 1
        if HAS_SORTER:
            sorted_index = tl.load(sorter + sorted_offsets, mask=search_active, other=0)
            if USE_INT32_INDEX:
                sorted_index = sorted_index.to(tl.int32)
            sorted_offsets = row_offsets + sorted_index

        mid_values = tl.load(
            sorted_sequence + sorted_offsets, mask=search_active, other=0
        )
        if RIGHT:
            advance = ~(mid_values > values_in)
        else:
            advance = ~(mid_values >= values_in)

        idx = tl.where(search_active & advance, next_idx, idx)
...
def _scalar_values_tensor(sorted_sequence, value):
    # Preserve Python scalar precision before ATen's promotion and narrowing.
    # Creating a narrow integer tensor directly rejects values that ATen casts.
    dtype = None
    if isinstance(value, float):
        dtype = torch.float64
    elif isinstance(value, int) and value >= 1 << 63:
        dtype = torch.uint64
    values = torch.tensor(
        value,
    ...
    return values.to(dtype=torch.result_type(sorted_sequence, value))
...
def searchsorted_scalar(
    ...
    *,
    ...
    values = _scalar_values_tensor(sorted_sequence, self)
    return _searchsorted_impl(
        sorted_sequence,
        values,
        out_int32=out_int32,
