...
def radix_sortbykey_scatter_kernel(
    ...
    for s in range(bins_segment):
        ...
        while bk >= 0:
            ...
            partial_prefix = tl.load(d_lookback + rd_lbk_offset)
...
def sort_by_key(key, value, valid_bits, generator=None):
    ...
    if n_elements > 2 * 1024:
        ...
        grid_hist = (
            ...
            bins // bins_per_sgement,
        )

        digit_hist_slice = torch.empty(
            (passes, bins + 1, grid_hist[0]),
            dtype=hist_dtype,
            device=key.device,
        )

        digit_hist = torch.empty(
            (num_portions, passes, bins + 1),
            dtype=hist_dtype,
            device=key.device,
        )
        ...
        digit_hist_slice = digit_hist_slice.cumsum(dim=1)
        ...
        increment = triton.cdiv(n_elements, 4)
        ...
        BLOCK_SIZE = triton.next_power_of_2(max(n_elements, 1))
        grid = (1,)
