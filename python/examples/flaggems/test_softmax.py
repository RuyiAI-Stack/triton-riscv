...
def test_softmax_backward(size):
    ...
    ref_grad = torch.autograd.grad(output, x, grad_output)[0]

    tri_grad = softmax_backward(grad_output, output, dim=-1, input_dtype=x.dtype)
...
def test_softmax_backward_out(size):
    ...
    torch.testing.assert_close(tri_grad, ref_grad, rtol=1e-4, atol=1e-4)
    torch.testing.assert_close(grad_input, ref_grad, rtol=1e-4, atol=1e-4)
