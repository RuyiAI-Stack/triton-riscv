...
def tl_max_tie_break_right(input, index, axis=None, keep_dims=False):
    return tl.reduce(
        (input, index),
        axis,
        maximum_with_index_tie_break_right,
        keep_dims=keep_dims,
    )
...
def add_base_max_kernel(
    ...
    if pid > 0:
        ...
        final_indices = tl.where(
            out_vals >= last_part_max_via_max,
            out_indices,
            last_part_max_index_via_max,
        )
...
def scan_then_fan_col(
    ...
    scan_part_max_kernel[grid](
        ...
        NEED_PARTIAL=need_partial,
        USE_OUT_INDICES=use_out_indices,
    )
...
@triton.jit
def cummax_sequential_kernel(inp, out, out_indices, B, C):
    pid_a = tl.program_id(0)
    pid_c = tl.program_id(1)
    ...
    prev_value = tl.full((), min_value, dtype=compute_dtype)
