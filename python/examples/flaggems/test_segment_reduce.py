...
def test_segment_reduce_lengths_views():
    storage = torch.tensor([1, 1, 1, 0, 0, 0, 2], dtype=torch.int64)
    data = torch.tensor([1.0, 2.0, 3.0])
    for lengths in (storage[:3], storage[::3]):
        torch.testing.assert_close(
            segment_reduce(data, "sum", lengths=lengths),
            torch.segment_reduce(data, "sum", lengths=lengths),
        )
    storage[0] = 0
    storage[3] = 1
    torch.testing.assert_close(
        segment_reduce(data, "sum", lengths=storage[::3]),
        torch.segment_reduce(data, "sum", lengths=storage[::3]),
    )
