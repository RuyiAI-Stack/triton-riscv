...
def _assert_adam_state_close(actual, expected, amsgrad):
    for name in ("params", "grads", "exp_avgs", "exp_avg_sqs", "state_steps"):
...
def test_fused_adam_inplace_scaled_gradients(skip_update):
    actual = _adam_state()
    expected = _clone_adam_state(actual)
    original_grad = actual["grads"][0]
    options = dict(
        lr=0.01,
        beta1=0.9,
        beta2=0.999,
        ...
        amsgrad=True,
        maximize=True,
        grad_scale=torch.tensor(2.0),
        found_inf=torch.tensor(float(skip_update)),
    )

    for state, op in ((actual, _fused_adam_), (expected, torch._fused_adam_)):
        op(
            state["params"],
            state["grads"],
            state["exp_avgs"],
    ...
    assert actual["grads"][0] is original_grad
