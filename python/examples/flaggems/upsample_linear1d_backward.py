...
def upsample_linear1d_backward_kernel(
    ...
    if reciprocal_scale == 0.0:
        ...
        center = x_in_f / reciprocal_scale
    else:
        center = (x_in_f + 0.5) / reciprocal_scale - 0.5
    ...
    for i in tl.static_range(-WINDOW, WINDOW + 1):
        ...
        if align_corners:
            x_real = x_out_f * reciprocal_scale
        else:
            x_real = (x_out_f + 0.5) * reciprocal_scale - 0.5
        x_real = tl.minimum(tl.maximum(x_real, 0.0), in_w_f - 1.0)
...
def upsample_linear1d_backward(
    ...
    if isinstance(scale_factors, (tuple, list)):
    ...
    BLOCK = 512
    if align_corners:
        reciprocal_scale = (in_w - 1) / (out_w - 1) if out_w > 1 else 0.0
    else:
        reciprocal_scale = (
            1.0 / scale if scale is not None and scale > 0 else in_w / out_w
        )

    if reciprocal_scale == 0.0:
        WINDOW = out_w
    else:
        # Include both interpolation neighbors, plus all outputs clamped to an
        # endpoint (explicit scales need not agree with the supplied size).
        if align_corners:
            first_center = 0.0
            last_center = (in_w - 1) / reciprocal_scale
        else:
            first_center = 0.5 / reciprocal_scale - 0.5
            last_center = (in_w - 0.5) / reciprocal_scale - 0.5
        WINDOW = (
            math.ceil(
                max(1.0 / reciprocal_scale, first_center, out_w - 1 - last_center)
            )
            + 2
        )
