...
from .topk import _get_finfo_val, get_dtype_max, get_dtype_min
...
def _nanmedian_kthvalue_fallback(inp, M, N):
    ...
    if torch.is_floating_point(inp):
        ...
        if inp.device.type == "npu" and inp.dtype in (
            torch.int32,
            torch.int64,
        ):
...
def _nanmedian_dim_impl(inp, dim, keepdim, out=None, use_ascend_float_select=True):
    ...
    if out is not None:
        if values is not out[0]:
            out[0].copy_(values)
        if indices is not out[1]:
            out[1].copy_(indices)
        values, indices = out
