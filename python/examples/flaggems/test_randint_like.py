...
@pytest.mark.parametrize("high", [2**31 + 1, 2**40, 2**62 + 1])
def test_randint_like_wide_int64_range(high):
    x = torch.empty(4096, dtype=torch.int64)
    result = randint_like(x, high, generator=torch.Generator().manual_seed(456))
