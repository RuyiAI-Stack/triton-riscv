...
def test_searchsorted_nan_values(right, use_sorter):
    boundaries = torch.tensor([1.0, 2.0, 3.0])
    sorter = None
    if use_sorter:
        boundaries = boundaries.flip(0)
        sorter = torch.argsort(boundaries)
    values = torch.tensor([float("nan"), -float("inf"), float("inf"), 2.0])
    expected = torch.searchsorted(boundaries, values, right=right, sorter=sorter)
    torch.testing.assert_close(
        searchsorted(boundaries, values, right=right, sorter=sorter), expected
    )


@pytest.mark.parametrize("right", [False, True])
@pytest.mark.parametrize("base", [2**30, 2**60])
def test_searchsorted_scalar_integer_precision(right, base):
    boundaries = torch.tensor([base, base + 1, base + 2], dtype=torch.int64)
    value = base + 1
    expected = torch.searchsorted(boundaries, value, right=right)
    torch.testing.assert_close(
        searchsorted_scalar(boundaries, value, right=right), expected
    )
    out = torch.empty((), dtype=torch.int64)
    result = searchsorted_scalar_out(boundaries, value, right=right, out=out)
    assert result is out
    torch.testing.assert_close(out, expected)


def test_searchsorted_scalar_double_precision():
    boundaries = torch.tensor([1.0, 2.0, 3.0], dtype=torch.float64)
    value = 1.99999999
    torch.testing.assert_close(
        searchsorted_scalar(boundaries, value, right=True),
        torch.searchsorted(boundaries, value, right=True),
    )
...
@pytest.mark.parametrize("right", [False, True])
def test_searchsorted_scalar_integer_conversion(dtype, value, right):
    boundaries = torch.tensor([1, 2, 3], dtype=dtype)
    expected = torch.searchsorted(boundaries, value, right=right)
    torch.testing.assert_close(
        searchsorted_scalar(boundaries, value, right=right), expected
    )
