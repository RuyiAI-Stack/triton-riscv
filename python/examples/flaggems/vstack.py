...
def vstack_kernel(
    ...
    end_idx = local_row * row_width.to(tl.int64)
    idx = (pid_x * BLOCK_SIZE + col_idx).to(tl.int64)
    offset_mask = idx < end_idx
    in_offset = intensor_ptr + idx
    row_offset = (total_row_offset + base_exc_row_idx) * row_width.to(tl.int64)
    out_offset = output_ptr + row_offset + idx
...
def vstack(tensors: list):
    ...
    # Contiguous singleton dimensions can retain arbitrary strides.
    row_width = math.prod(output_shape[1:])
