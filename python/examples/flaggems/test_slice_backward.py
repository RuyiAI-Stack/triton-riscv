...
def test_slice_backward(size):
    ...
    ref_grad_input = torch.zeros(input_sizes, dtype=torch.float32, device="cpu")
    ref_grad_input[start:end:step] = grad_output

    tri_grad_input = slice_backward(grad_output, input_sizes, dim, start, end, step)

    torch.testing.assert_close(tri_grad_input, ref_grad_input, rtol=1e-4, atol=1e-4)
...
def test_slice_backward_2d(size):
    ...
    grad_output = torch.randn(grad_output_shape, dtype=torch.float32, device="cpu")
