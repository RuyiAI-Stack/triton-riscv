...
def _add_internal(A, B, alpha=1):
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor):
        ...
        A_c = A.contiguous()
        out_dtype = torch.result_type(A_c, torch.tensor(B))
        out = torch.empty_like(A_c, dtype=out_dtype)
        n_elements = A_c.numel()
        BLOCK_SIZE = 1024
        grid = (triton.cdiv(n_elements, BLOCK_SIZE),)
        ...
        B_c = B.contiguous()
        out_dtype = torch.result_type(torch.tensor(A), B_c)
        out = torch.empty_like(B_c, dtype=out_dtype)
...
def add(A, B, *, alpha=1):
    ...
    if A_is_complex or B_is_complex:
        if A_is_complex and B_is_complex:
            ...
            out_real = _add_internal(Ar, Br, alpha)
    ...
    return _add_internal(A, B, alpha)


def add_(A, B, *, alpha=1):
    res = add(A, B, alpha=alpha)
    A.copy_(res)
    return A
