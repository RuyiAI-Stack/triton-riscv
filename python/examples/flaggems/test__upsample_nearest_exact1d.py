...
def test__upsample_nearest_exact1d_size(shape, out_size):
    ...
    ref_out = torch.nn.functional.interpolate(x, size=out_size, mode="nearest-exact")
...
def test__upsample_nearest_exact1d_scale(shape, scale):
    ...
    ref_out = torch.nn.functional.interpolate(
        x, scale_factor=scale, mode="nearest-exact"
    )
...
def test__upsample_nearest_exact1d_out(shape, out_size):
    torch.manual_seed(0)
    x = torch.randn(shape, device="cpu", dtype=torch.float32)
    out = torch.empty((shape[0], shape[1], out_size), device="cpu", dtype=torch.float32)
