...
@pytest.mark.parametrize("shape", [(10, 5), (32, 10)])
def test_nllloss_backward(shape):
    N, C = shape
    inp = F.log_softmax(
    ...
    ref_out = F.nll_loss(inp, tgt, reduction="mean")
...
@pytest.mark.parametrize("shape", [(2, 5, 4, 4)])
def test_nll_loss2d_backward(shape):
    torch.manual_seed(0)
    N, C, H, W = shape
    log_probs = F.log_softmax(torch.randn(shape, dtype=torch.float32), dim=1)
    ...
    tri_out, total_weight = nll_loss2d_forward(log_probs2, tgt, reduction=1)
    tri_dx = nll_loss2d_backward(
        grad_out, log_probs2, tgt, reduction=1, total_weight=total_weight
    )
