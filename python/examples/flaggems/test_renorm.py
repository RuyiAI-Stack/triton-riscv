...
def test_renorm_infinity_norm(dim, inplace):
    x = torch.tensor([[2.0, -4.0, 0.0], [0.0, 0.0, 0.0]])
    expected = torch.renorm(x, p=float("inf"), dim=dim, maxnorm=1.0)
    op = renorm_ if inplace else renorm
    actual = op(x, p=float("inf"), dim=dim, maxnorm=1.0)
    if inplace:
        assert actual is x
