...
@pytest.mark.parametrize(
    "N, C, H, W, out_channels, kernel_size, stride, padding, dilation, groups",
    [
        (2, 4, 16, 16, 8, 3, 1, 1, 1, 1),
        (1, 3, 32, 32, 6, 5, 2, 2, 1, 1),
        (2, 4, 16, 16, 8, 3, 1, 1, 2, 1),
        (1, 8, 16, 16, 8, 3, 1, 1, 1, 2),
        (1, 4, 16, 16, 8, 3, 1, 0, 1, 1),
        (2, 4, 16, 16, 8, 3, 1, "same", 1, 1),
        (2, 4, 16, 16, 8, 3, 1, "valid", 1, 1),
        # Test required sizes: 512, 1023, 1024
        (1, 1, 512, 512, 1, 3, 1, 1, 1, 1),
        # Use small out_channels to avoid OOM
        (1, 1, 1023, 1023, 1, 3, 1, 1, 1, 1),
        (1, 1, 1024, 1024, 1, 3, 1, 1, 1, 1),
    ],
)
...
def test_conv2d(
    ...
    weight = torch.randn(
        (out_channels, C // groups, kernel_size, kernel_size),
        dtype=torch.float32,
        device=device,
        requires_grad=True,
    )
