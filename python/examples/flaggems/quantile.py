...
def quantile_kernel(
    ...
    q_lower = tl.math.floor(q_block).to(tl.int32)
    q_upper = tl.math.ceil(q_block).to(tl.int32)
    ...
    if interpolation == "linear":
        ...
        q_floor = tl.math.floor(q_block)
        q_ceil = tl.math.ceil(q_block)
        diff = q_block - q_floor
        # Round to even for .5 ties
        is_even = (q_floor.to(tl.int32) % 2) == 0
        q_round = tl.where(
            diff == 0.5,
...
def quantile_bitonic_kernel(
    ...
    q_scaled = q_vals * (M - 1)
    q_lower = tl.math.floor(q_scaled).to(tl.int32)
    q_upper = tl.math.ceil(q_scaled).to(tl.int32)
    ...
    if interpolation == "linear":
        ...
        q_floor = tl.math.floor(q_scaled)
        q_ceil = tl.math.ceil(q_scaled)
        diff = q_scaled - q_floor
        is_even = (q_floor.to(tl.int32) % 2) == 0
        q_round = tl.where(
            diff == 0.5,
            tl.where(is_even, q_floor, q_ceil),
...
def quantile(
    ...
    if M <= MAX_BITONIC_M:
        ...
        BLOCK_Q = heur_block_q({"Q": Q})
        BLOCK_N = heur_block_n({"N": N})
