...
@pytest.mark.parametrize("contains_non_finite", [False, True])
def test_amp_unscale_float64_dynamic_range(contains_non_finite):
    values = [1e40, 1e-50, -1e40, -1e-50]
    if contains_non_finite:
        values += [float("inf"), float("nan")]
    actual = torch.tensor(values, dtype=torch.float64)
    expected = actual.clone()
    found_inf = torch.zeros(())
    ref_found_inf = torch.zeros(())
    inv_scale = torch.tensor(0.5)

    _amp_foreach_non_finite_check_and_unscale_([actual], found_inf, inv_scale)
    torch._amp_foreach_non_finite_check_and_unscale_(
        [expected], ref_found_inf, inv_scale
    )

    torch.testing.assert_close(actual, expected, rtol=1e-14, atol=0, equal_nan=True)
    torch.testing.assert_close(found_inf, ref_found_inf)
