...
def _get_hardcoded_heuristic_config(config_str):
    if config_str == "mha_block_128":
        return {
...
def mha_varlan_fwd(
    ...
    cfg = _get_hardcoded_heuristic_config(varlen_fwd_config_str)
    cfg_params = {
...
def mha_fwd(
    ...
    if q.device.type == "cpu":
        ...
        softmax_scale = (
            softmax_scale if softmax_scale is not None else 1.0 / (q.shape[-1] ** 0.5)
        )
        out = (
        ...
        lse = torch.logsumexp(
            torch.matmul(
                q.transpose(1, 2).to(torch.float32),
                k.transpose(1, 2).transpose(-2, -1).to(torch.float32),
            )
            * softmax_scale,
            dim=-1,
        )
        philox_args = torch.empty((2,), dtype=torch.int64, device=q.device)
        unused = torch.empty((), dtype=torch.int64, device=q.device)
        p = torch.empty((), device=q.device)
        return out, q, k, v, lse, philox_args, unused, p
    ...
    def dispatch(B, H, Q, K, D, params):
        ...
        if not is_dropout and not is_local and not disable_splitkv:
            ...
            if n_splits > 1:
                ...
                def grid(args):
                    return (triton.cdiv(B * H * Q, BLOCK_M),)
