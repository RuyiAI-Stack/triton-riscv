...
def test_shifted_chebyshev_u_broadcast_and_real_domain(dtype, n):
    x = torch.tensor([[-2.0], [0.0], [0.5], [1.0], [2.0]], dtype=dtype)
    actual = special_shifted_chebyshev_polynomial_u(x, n)
    expected = torch.special.shifted_chebyshev_polynomial_u(x, n)
    torch.testing.assert_close(actual, expected)


@pytest.mark.parametrize("n", [-2, -0.5, 0.5, 2.9])
def test_shifted_chebyshev_u_degree_conversion(n):
    x = torch.tensor([-2.0, -1.0, 0.5, 2.0])
    torch.testing.assert_close(
        special_shifted_chebyshev_polynomial_u(x, n),
        torch.special.shifted_chebyshev_polynomial_u(x, n),
    )


def test_shifted_chebyshev_u_inplace_outside_interval():
    x = torch.tensor([-1.0, 2.0])
    expected = torch.special.shifted_chebyshev_polynomial_u(x, 2)
    assert special_shifted_chebyshev_polynomial_u_(x, 2) is x
