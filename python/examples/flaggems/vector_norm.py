...
@triton.jit
def _pow(x, y):
    return tl.exp2(tl.log2(tl.abs(x) + 1e-10) * y)
...
def v_norm_kernel(
    ...
    sum = tl.sum(_sum, axis=1)
    out = _pow(sum, 1 / ord)[:, None]
    tl.store(Out, out, row_mask)


@triton.jit
def vec_norm_kernel_1(
    X,
    Mid,
    ord,
    M,
    ...
    x = tl.load(X, mask=mask, other=0.0).to(tl.float32)
    mid = tl.sum(_pow(tl.abs(x), ord))
    tl.store(Mid, mid)


@triton.jit
def vec_norm_kernel_2(
    Mid,
    Out,
    ord,
    MID_SIZE,
...
def vector_norm(x, ord=2, dim=None, keepdim=False, dtype=None):
    ...
    if (not dim) or len(dim) == x.ndim:
        ...
        if ord == 2:
            ...
            vec_norm_kernel_1[(MID_SIZE,)](x, mid, ord, M, BLOCK_SIZE)
            vec_norm_kernel_2[(1,)](mid, out, ord, MID_SIZE, BLOCK_MID)
    else:
        shape = list(x.shape)
