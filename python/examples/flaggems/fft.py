...
def fft_kernel_triton(
    ...
    if LOG_N % 2 == 1:
        ...
        # tl.debug_barrier()  # removed: gpu.barrier not supported on triton-riscv
    ...
    if LOG_N % 2 == 1:
        for r4 in tl.static_range((LOG_N - 1) // 2):
            ...
            out_real = tl.where(
                m0,
                o0_real,
                tl.where(m1, o1_real, tl.where(m2, o2_real, o3_real)),
            )
            out_imag = tl.where(
                m0,
                o0_imag,
                tl.where(m1, o1_imag, tl.where(m2, o2_imag, o3_imag)),
            )
