...
def _launch_roll_flat_kernel(
    ...
    def grid(meta):
        return (triton.cdiv(numel, meta["BLOCK"]),)
...
def _roll_single_dim_kernel(
    ...
    source_dim_index = (dim_index + dim_size - (shift % dim_size)) % dim_size
    source_offsets = (
        (offsets // (dim_stride * dim_size)) * (dim_stride * dim_size)
        + source_dim_index * dim_stride
        + offsets % dim_stride
    )
...
@triton.jit
def _roll_last_dim_kernel(
    inp_ptr, out_ptr, numel, width: tl.constexpr, shift, BLOCK: tl.constexpr
):
    offsets = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offsets < numel
    column = offsets % width
    row_start = offsets - column
    source_column = (column + width - (shift % width)) % width
    source_offsets = row_start + source_column
    values = tl.load(inp_ptr + source_offsets, mask=mask, other=0)
    tl.store(out_ptr + offsets, values, mask=mask)
