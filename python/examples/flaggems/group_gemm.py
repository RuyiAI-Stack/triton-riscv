...
_support_device_tensor_descriptor = hasattr(tl, "make_tensor_descriptor")
make_tensor_descriptor_fn = (
    tl.make_tensor_descriptor if _support_device_tensor_descriptor else None
)
...
def grouped_gemm_tma_kernel(
    ...
    for g in range(group_size):
        ...
        if tile_idx >= last_problem_end and tile_idx < current_problem_end:
            ...
            out_ptr = tl.load(group_out_ptrs + g).to(
            ...
            loop_count = (
                current_problem_end - tile_idx + total_grid - 1
            ) // total_grid
            for _ in tl.range(loop_count):
                ...
                for kk in range(0, tl.cdiv(gk, BLOCK_K)):
                    ...
                    accumulator = tl.dot(
                        a, b, acc=accumulator, allow_tf32=False
                    )
...
def group_gemm(group_A, group_B, group_C, offs_table, alpha=1, beta=0):
    ...
    d_g_sizes = torch.tensor(
        group_sizes, dtype=torch.int32, device=group_A.device
    )
    d_g_lds = torch.tensor(group_lds, dtype=torch.int32, device=group_A.device)
    NUM_SMS = _get_sm_count()

    if _support_device_tensor_descriptor and _support_tma:
...
def group_mm(
    ...
    NUM_SMS = _get_sm_count()
    C = A.new_empty(M, N)
    if _support_host_tensor_descriptor and _support_tma:
        dummy_block = [1, 1]
