...
def _unique_dim_group_id_kernel(
    ...
    prev = tl.load(composite_ptr + prev_offsets, mask=mask & (offsets > 0), other=cur)
...
def _unique_dim_gather_output(
    ...
    output_shape = (
        *tuple(input_shape[:dim]),
        num_unique,
        *tuple(input_shape[dim + 1 :]),
    )
    if num_unique == 0:
...
def _unique_dim_inverse_from_permutation(
