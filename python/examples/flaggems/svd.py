...
def _is_float32_cuda_matrix(input):
    return input.dtype == torch.float32 and input.dim() >= 2


def _is_low_precision_cuda_matrix(input):
    return input.dtype in (torch.float16, torch.bfloat16) and input.dim() >= 2
...
def _complex_svd_pick_orthonormal_v_kernel(
    ...
    for c in tl.static_range(0, 16):
        ...
        for p in tl.static_range(0, c):
            ...
            coeff_r = tl.sum(
                tl.where(row_mask, prev_r * cur_r + prev_i * cur_i, 0.0),
                axis=0,
            )
            coeff_i = tl.sum(
                tl.where(row_mask, prev_r * cur_i - prev_i * cur_r, 0.0),
                axis=0,
            )
