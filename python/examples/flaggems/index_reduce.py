...
INDEX_REDUCE_BLOCK_M = 4
INDEX_REDUCE_BLOCK_N = 256
INDEX_REDUCE_FLAT_BLOCK = 256
    ...
    if REDUCE == 0:
        ...
        while start < N:
            ...
            if REDUCE == 1:
                ...
                acc = tl.maximum(
                    acc,
                    tl.max(tl.where(matched, values, float("-inf")), axis=0),
                )
            else:
                acc = tl.minimum(
                    acc,
                    tl.min(tl.where(matched, values, float("inf")), axis=0),
                )
            start += BLOCK_N
...
def _validate_args(inp, dim, index, source, reduce):
    ...
    assert inp.is_floating_point(), (
        "index_reduce_(): Expected self to be floating point"
    )
    assert source.dtype == inp.dtype, (
    ...
    assert all(inp.size(i) == source.size(i) or i == dim for i in range(inp.ndim)), (
...
def index_reduce_(inp, dim, index, source, reduce, *, include_self=True):
    ...
    if (
        ...
        grid = (triton.cdiv(total, INDEX_REDUCE_FLAT_BLOCK),)
        _index_reduce_contiguous_flat_kernel[grid](
            out,
            index,
            source,
            count,
            ...
            use_cas,
            index_major,
            BLOCK=INDEX_REDUCE_FLAT_BLOCK,
        )

        if reduce == "mean":
            acc = out
            grid = (triton.cdiv(inp.numel(), INDEX_REDUCE_FLAT_BLOCK),)
            _index_reduce_mean_finalize_kernel[grid](
                inp,
                acc,
                inp,
                count,
    ...
    if "" == "ascend" and _index_is_unique(index, out_n):
        out = inp_work
        grid = (
            triton.cdiv(M, INDEX_REDUCE_BLOCK_M),
            triton.cdiv(N, INDEX_REDUCE_BLOCK_N),
        )
    ...
    grid = (triton.cdiv(total, INDEX_REDUCE_FLAT_BLOCK),)
    _index_reduce_flat_kernel[grid](
        out,
        index,
        source_compute,
        count,
    ...
    if reduce == "mean":
        result = out
        grid = (triton.cdiv(inp_compute.numel(), INDEX_REDUCE_FLAT_BLOCK),)
        _index_reduce_mean_finalize_kernel[grid](
            result,
            out,
            inp_compute,
