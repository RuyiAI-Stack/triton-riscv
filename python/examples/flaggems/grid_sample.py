...
def grid_sample_2d_nearest_zeros_kernel(
    ...
    # Clamp corner indices to avoid invalid pointers even when masked
    ...
    input_offset = (
        n * C * H_in * W_in + c * H_in * W_in + y_idx_clamped * W_in + x_idx_clamped
    )
...
def grid_sample_2d_bilinear_zeros_kernel(
    ...
    # Clamp corner indices to avoid invalid pointers even when masked
    x0_idx = tl.maximum(0, tl.minimum(x0_int, W_in - 1))
    y0_idx = tl.maximum(0, tl.minimum(y0_int, H_in - 1))
    x1_idx = tl.maximum(0, tl.minimum(x1_int, W_in - 1))
    y1_idx = tl.maximum(0, tl.minimum(y1_int, H_in - 1))

    # Load 4 corner pixels with zeros padding
    # Input shape: (N, C, H_in, W_in)
    input_base = n * C * H_in * W_in + c * H_in * W_in

    offset_00 = input_base + y0_idx * W_in + x0_idx
    offset_01 = input_base + y0_idx * W_in + x1_idx
    offset_10 = input_base + y1_idx * W_in + x0_idx
    offset_11 = input_base + y1_idx * W_in + x1_idx
...
def grid_sample_2d_bicubic_zeros_kernel(
    ...
    x0 = tl.math.floor(x) - 1
    y0 = tl.math.floor(y) - 1
